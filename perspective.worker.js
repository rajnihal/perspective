...
module.exports = __webpack_require__.p + "48577096dd975add.wasm";
...
for(const callback of this._delete_callbacks){callback();}};/**
     ...
     */function parse_expression_strings(expressions){let validated_expressions=[];const expression_idx_map={};for(let expression_string of expressions){// Map of column names to column IDs, so that we generate
...
let expression_alias;let alias_match=expression_string.match(/^\/\/(?<alias>.+?)\n/);if(alias_match?.groups?.alias){expression_alias=alias_match.groups.alias.trim();}// If an alias does not exist, the alias is the expression itself.
if(!expression_alias||expression_alias.length==0){expression_alias=expression_string;}// Replace `true` and `false` reserved words with symbols
let parsed_expression_string=expression_string.replace(/([a-zA-Z_]+[a-zA-Z0-9_]*)/g,match=>{if(match=="true"){return"True";}else if(match=="false"){return"False";}else{return match;}});parsed_expression_string=parsed_expression_string.replace(/\"(.*?[^\\])\"/g,(_,cname)=>{// If the column name contains escaped double quotes, replace
...
// replaced.
parsed_expression_string=parsed_expression_string.replace(/(replace_all|replace)\(.*?,\s*(intern\(\'(.*)\'\)),.*\)/g,replace_interned_param);const validated=[expression_alias,expression_string,parsed_expression_string,column_id_map];// Check if this expression is already in the array, if so then
...
if(expression_idx_map[expression_alias]!==undefined){const idx=expression_idx_map[expression_alias];validated_expressions[idx]=validated;}else{validated_expressions.push(validated);expression_idx_map[expression_alias]=validated_expressions.length-1;}}return validated_expressions;}/**
     ...
     */table.prototype.validate_expressions=function(expressions,override=true){const validated={expression_schema:{},expression_alias:{},errors:{}};if(!expressions||expressions.length===0)return validated;expressions=parse_expression_strings(expressions);// Transform Array into a C++ vector that can be passed through
// Emscripten.
...
config["aggregates"]={};config["columns"]=[];for(const agg of _config["aggregate"]){config["aggregates"][agg["column"]]=agg["op"];config["columns"].push(agg["column"]);}}else if(CONFIG_VALID_KEYS.indexOf(key)>-1){config[key]=_config[key];}else{throw new Error(`Unrecognized config parameter "${key}"`);}}config.group_by=config.group_by||[];config.split_by=config.split_by||[];config.aggregates=config.aggregates||{};config.filter=config.filter||[];config.sort=config.sort||[];config.expressions=config.expressions||[];const table_schema=this.schema();if(config.expressions.length>0){config.expressions=parse_expression_strings(config.expressions);}if(config.columns===undefined){// If columns are not provided, use all columns
config.columns=this.columns();if(config.expressions.length>0){for(const expr of config.expressions){config.columns.push(expr[0]);}}}// convert date/datetime filters to Date() objects, so they are parsed
// as local time
...
return new table(_Table,options.index,options.limit,overridden_types);}catch(e){if(_Table){_Table.delete();}console.error(`Table initialization failed: ${e}`);throw e;}}};for(let prop of Object.keys(constants_namespaceObject)){perspective[prop]=constants_namespaceObject[prop];}/**
     ...
     */class WebWorkerServer extends Server{/**
         ...
         */init(msg){if(typeof WebAssembly==="undefined"){throw new Error("WebAssembly not supported");}else{__MODULE__({wasmBinary:msg.buffer,wasmJSMethod:"native-wasm",locateFile:x=>x}).then(mod=>{__MODULE__=mod;__MODULE__.init();super.init(msg);});}}}/**
